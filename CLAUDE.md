...
| `.claude/rules/agent-patterns.mdc` | workflow agents (**any agent failure fails the workflow**), standalone interview prep + CV optimizer + Practice Interview + Hiring Outreach + **Job Finder resolver**, `workflow_preferences`, BYOK model override, **Company Research — `_has_usable_company_name` / unnamed-posting / disambiguation / staffing / `research_quality`** |
...
| `.claude/rules/job-finder-feature.mdc` | Job Finder chat — company careers discovery, ATS providers, orchestrator, `/api/v1/job-finder` |
