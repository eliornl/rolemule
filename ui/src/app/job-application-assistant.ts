...
export class JobApplicationAssistant {
  apiBaseUrl: string;
  token: string | null;
  user: User;
  ...
  constructor() {
    ...
    setupEventListeners() {
        ...
        document.addEventListener('submit', (event) => {
            ...
            if (target?.matches('.ajax-form')) {
                ...
                this.handleFormSubmission(target as HTMLFormElement);
            }
        });
        ...
        document.addEventListener('change', (event) => {
            ...
            if (target?.matches('.file-upload-input')) {
                this.handleFileUpload(target as HTMLInputElement);
            }
        });
        ...
        document.addEventListener('drop', (event) => {
            ...
            if (uploadArea) {
                ...
                const fileInput = (uploadArea.querySelector('.file-upload-input') as HTMLInputElement | null);
    ...
    async checkAuthStatus() {
        ...
        try {
            const response = (await this.apiCall('/auth/verify', 'GET')) as ApiFormResponse;
            if (!response.success) {
                this.logout();
            }
        } catch (error) {
        ...
        try {
            ...
            if (
                ...
                try {
                    ...
                    try {
                        retryResult = (await retryResponse.json()) as Record<string, unknown>;
                    } catch {
                    ...
                    if (!retryResponse.ok) {
                        const retryErr = new Error(
                            String(retryResult.message || retryResult.error || `HTTP ${retryResponse.status}`),
                        ) as ApiError;
                        if (retryResult.error_code) {
                            retryErr.errorCode = String(retryResult.error_code);
                        }
                        throw retryErr;
                    }
            ...
            if (!response.ok) {
                const apiErr = new Error(
                    String(result.message || result.detail || result.error || `HTTP ${response.status}`),
                ) as ApiError;
                if (result.error_code) {
                    apiErr.errorCode = String(result.error_code);
                }
                if (result.details) {
                    apiErr.details = result.details;
                }
                throw apiErr;
            }
    ...
    async handleFormSubmission(form: HTMLFormElement) {
        ...
        try {
            ...
            let data: FormData | Record<string, FormDataEntryValue> = formData;
            ...
            const response = (await this.apiCall(endpoint, method, data)) as ApiFormResponse;
     ...
     */
    async handleFileUpload(input: HTMLInputElement) {
     ...
     * @param {File} file
     */
    displayFileInfo(input: HTMLInputElement, file: File) {
     ...
     */
    setButtonLoading(button: HTMLButtonElement | HTMLElement, loading: boolean, originalText: string | null = null) {
     ...
     */
    showNotification(message: string, type: NotificationType = 'info', duration = 5000) {
     ...
     * });
     */
    showModal(title: string, content: string, options: ModalOptions = {}) {
        ...
        // @ts-ignore
        const bs2 = typeof bootstrap !== 'undefined' ? (bootstrap) : null;
        if (bs2) {
     ...
     * }
     */
    confirm(message: string, title = 'Confirm', options: ModalOptions = {}) {
     ...
     */
    updateProgress(selector: string, percentage: number) {
        const progressBar = document.querySelector(selector) as HTMLElement | null;
        if (progressBar) {
     ...
     */
    animateCounter(element: HTMLElement, start: number, end: number, duration = 2000) {
     ...
     */
    debounce(func: (...args: unknown[]) => void, wait: number, immediate = false) {
     ...
     * @param {number} limit
     */
    throttle(func: (...args: unknown[]) => void, limit: number) {
     ...
     * @param {number} [offset]
     */
    scrollTo(element: string | HTMLElement, offset = 0) {
     ...
     */
    updateUrl(params: Record<string, string | number | null | undefined>, replaceState = false) {
...
export function initJobApplicationAssistant(): JobApplicationAssistant {
  const app = new JobApplicationAssistant();
  window.app = app;
  if (window.eventBus && window.BusEvents) {
    const bus = window.eventBus;
    const events = window.BusEvents;
    bus.on(events.NOTIFY_SUCCESS, (e) => {
      const data = e.data as { message?: string } | undefined;
      app.showNotification(String(data?.message ?? ''), 'success');
    });
    bus.on(events.NOTIFY_ERROR, (e) => {
      const data = e.data as { message?: string } | undefined;
      app.showNotification(String(data?.message ?? ''), 'error');
    });
    bus.on(events.NOTIFY_WARNING, (e) => {
      const data = e.data as { message?: string } | undefined;
      app.showNotification(String(data?.message ?? ''), 'warning');
    });
    bus.on(events.NOTIFY_INFO, (e) => {
      const data = e.data as { message?: string } | undefined;
      app.showNotification(String(data?.message ?? ''), 'info');
    });
  }
  return app;
}
