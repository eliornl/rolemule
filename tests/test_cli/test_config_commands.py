...
def test_config_show_human(invoke, rolemule_home) -> None:
    result = invoke("config")
    assert result.exit_code == 0
    assert "base_url:" in result.stdout


def test_config_show_json(invoke, rolemule_home) -> None:
...
def test_config_set_base_url(invoke, rolemule_home) -> None:
    result = invoke("config", "set", "--base-url", "https://apply.example.com")
    assert result.exit_code == 0
    assert load_config().base_url == "https://apply.example.com"


def test_config_set_requires_option(invoke, rolemule_home) -> None:
