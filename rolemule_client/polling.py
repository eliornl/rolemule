...
class CvPollTimeout(TimeoutError):
    """Raised when CV optimization polling times out or fails without a result."""
    ...
    def __init__(
        ...
        *,
        ...
        self.failed = failed
        if failed:
            super().__init__(f"CV optimization failed for session {session_id}")
        else:
            super().__init__(f"CV optimization for session {session_id} did not complete in time")


def wait_for_cv_optimization(
    get_status: Callable[[], Dict[str, Any]],
    *,
    interval_seconds: float = 3.0,
    timeout_seconds: float = 600.0,
    ...
    """Poll until CV optimization has a result and is no longer running."""
    deadline = time.monotonic() + timeout_seconds
    last: Dict[str, Any] = {}
    ...
    while True:
        ...
        if last.get("has_result") and not last.get("is_running"):
            return last

        if not last.get("is_running") and not last.get("has_result"):
            session_id = str(last.get("session_id") or "")
            raise CvPollTimeout(session_id, last_status=last, failed=True)

        if time.monotonic() >= deadline:
            session_id = str(last.get("session_id") or "")
            raise CvPollTimeout(session_id, last_status=last)
